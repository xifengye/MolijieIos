...
- (void)visibleViewDelegateForIndex:(NSUInteger)index{
    ...
    if (_prePageIndex != index) {
        if ([_delegate respondsToSelector:@selector(slideHiddenView:forIndex:)]) {
            [_delegate slideHiddenView:[self visibleCellForIndex:_prePageIndex] forIndex:_prePageIndex];
        }
